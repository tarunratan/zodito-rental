...
const STATUS_COLORS: Record<string, string> = {
  ...
  payment_failed: 'bg-muted/10 text-muted',
};
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-1.5" onClick={e => e.stopPropagation()}>
                          {b.status === 'confirmed' && (
                            <button
                              onClick={() => setActionModal({ id: b.id, action: 'ongoing' })}
                              disabled={loading === b.id}
                              className="text-xs px-2 py-1 bg-accent/10 text-accent rounded hover:bg-accent/20 transition-colors disabled:opacity-50"
                            >
                              Mark Pickup
                            </button>
                          )}
                          {b.status === 'ongoing' && (
                            <button
                              onClick={() => setActionModal({ id: b.id, action: 'completed' })}
                              disabled={loading === b.id}
                              className="text-xs px-2 py-1 bg-success/10 text-success rounded hover:bg-success/20 transition-colors disabled:opacity-50"
                            >
                              Mark Return
                            </button>
                          )}
                          {['confirmed', 'ongoing', 'pending_payment'].includes(b.status) && (
                            <button
                              onClick={() => { setActionModal({ id: b.id, action: 'cancelled' }); setActionNotes(''); }}
                          ...
                          {b.status === 'cancelled' && b.payment_status === 'paid' && (
                            <button
                              onClick={() => setActionModal({ id: b.id, action: 'refunded' })}
                        ...
                        <td colSpan={7} className="px-4 py-4">
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                            ...
                            <div>
                              ...
                              {b.cancelled_at && <p className="text-xs text-danger">Cancelled: {fmt(b.cancelled_at)}</p>}
          ...
          <div className="bg-white dark:bg-primary rounded-xl shadow-2xl w-full max-w-sm p-5 space-y-4">
            <h3 className="font-semibold capitalize">
              {actionModal.action === 'ongoing' ? 'Mark as Picked Up?' :
               ...
               actionModal.action === 'cancelled' ? 'Cancel Booking?' :
               'Process Refund?'}
            </h3>
            {['cancelled', 'completed'].includes(actionModal.action) && (
              <textarea
                ...
                placeholder={actionModal.action === 'cancelled' ? 'Cancellation reason (optional)' : 'Return notes, damage, KM details (optional)'}
              />
            )}
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                className={`px-4 py-2 text-sm text-white rounded-lg ${
                  actionModal.action === 'cancelled' ? 'bg-danger hover:bg-danger/90' :
