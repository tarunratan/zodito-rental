  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={() => { setShowForm(true); setForm({ ...EMPTY_FORM }); setError(null); }} className="btn-accent text-sm px-4 py-2">
          + Add Expense
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <div className="card p-4">
          ...
          <p className="text-xl font-bold text-danger">{rupee(grandTotal)}</p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {bikeSummary.slice(0, 8).map(([bikeId, { name, total }]) => (
              <div key={bikeId} className="p-3 bg-bg rounded-lg">
                ...
                <p className="font-semibold text-danger">{rupee(total)}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        ...
        {(filterBike !== 'all' || filterCat !== 'all') && (
          <div className="flex items-center gap-2 text-sm text-muted">
            ...
            <span className="font-semibold text-danger">{rupee(totalFiltered)}</span>
          </div>
        )}
      </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.map(e => (
                  <tr key={e.id} className="border-b border-border last:border-0 hover:bg-bg/40 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right font-semibold text-danger">{rupee(Number(e.amount))}</td>
              ...
              <tfoot>
                <tr className="border-t-2 border-border bg-bg/60">
                  ...
                  <td className="px-4 py-3 text-right font-bold text-danger text-sm">{rupee(totalFiltered)}</td>
                  <td />
                </tr>
              </tfoot>
            </table>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center p-4 overflow-y-auto">
          <div className="bg-white dark:bg-primary rounded-xl shadow-2xl w-full max-w-md my-8">
            ...
            <div className="px-5 py-4 border-t border-border flex justify-end gap-3">
              ...
              <button
                onClick={save}
                disabled={!form.bike_id || !form.description.trim() || !form.amount || saving}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-primary rounded-xl shadow-2xl w-full max-w-sm p-5 space-y-4">
            ...
            <div className="flex justify-end gap-3">
              ...
              <button onClick={() => del(deleteId)} className="px-4 py-2 text-sm bg-danger text-white rounded-lg hover:bg-danger/90">Delete</button>
            </div>
          </div>
        </div>
      )}
