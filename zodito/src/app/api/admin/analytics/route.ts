...
type BookingRow = { id: string; status: string; payment_status: string; total_amount: string; platform_commission: string; created_at: string; bike_id: string };
type BikeRow = { id: string; listing_status: string; is_active: boolean; frozen_until: string | null; model: { display_name: string } | null };
type UserRow = { id: string; kyc_status: string };
type VendorRow = { id: string; status: string };
type ExpenseRow = { id: string; bike_id: string; category: string; amount: string };

export async function GET() {
  try {
    if (!isMockMode()) await requireAdmin();
    if (isMockMode()) return NextResponse.json(mockData());
    ...
    const [allBookings, allBikes, kycResult, vendorResult, expenses] = await Promise.all([
      ...
      supabase.from('bike_expenses').select('id, bike_id, category, amount, expense_date'),
    ]);

    const bookings: BookingRow[] = (allBookings.data ?? []) as BookingRow[];
    const bikes: BikeRow[] = (allBikes.data ?? []) as BikeRow[];
    const users: UserRow[] = (kycResult.data ?? []) as UserRow[];
    const vendors: VendorRow[] = (vendorResult.data ?? []) as VendorRow[];
    const expenseRows: ExpenseRow[] = (expenses.data ?? []) as ExpenseRow[];
...
function mockData() {
  return {
    ...
    bikePnl: [
      { id: '1', name: 'Royal Enfield 350', revenue: 34500, expenses: 4200, net: 30300 },
      { id: '2', name: 'Honda Activa 6G', revenue: 22300, expenses: 1800, net: 20500 },
    ],
  };
}
